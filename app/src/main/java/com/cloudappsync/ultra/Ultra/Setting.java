...
public class Setting extends AppCompatActivity {
    ...
    private RadioButton ftpFolderRadio, ftpZipRadio, urlZipRadio, apiRadio, indexRadio, parseRadio;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ftpFolderRadio = findViewById(R.id.ftpFolderRadio);
        ftpZipRadio = findViewById(R.id.ftpZipRadio);
        urlZipRadio = findViewById(R.id.urlZipRadio);
        apiRadio = findViewById(R.id.apiRadio);
        indexRadio = findViewById(R.id.indexRadio);
    ...
    private void initSyncModeDefaults() {
        ...
        syncModeRadio.setOnCheckedChangeListener((group, checkedId) -> {
            ...
            switch (checkedId){
                ...
                case R.id.ftpFolderRadio:

                    //create table
                    String licenceFTPFolder = Paper.book().read(Common.COMPANY_ID) + "_" + Paper.book().read(Common.LICENCE_ID) + "_FOLDER";
                    new Database(getBaseContext()).createTable(licenceFTPFolder);

                    Paper.book().write(Common.CURRENT_SYNC_TYPE, Common.SYNC_TYPE_FTP_FOLDER);
                    Paper.book().write(Common.CURRENT_DB_LICENCE, licenceFTPFolder);
                    break;
                ...
                case R.id.indexRadio:
                    //create table
                    String licenceIndexFolder = Paper.book().read(Common.COMPANY_ID) + "_" + Paper.book().read(Common.LICENCE_ID) + "_INDEX";
                    new Database(getBaseContext()).createTable(licenceIndexFolder);

                    Paper.book().write(Common.CURRENT_SYNC_TYPE, Common.SYNC_TYPE_INDEX);
                    Paper.book().write(Common.CURRENT_DB_LICENCE, licenceIndexFolder);
        ...
        domainSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (!parent.getItemAtPosition(position).toString().equals("Custom Domains")) {
                    ...
                    //create dialog
                    android.app.AlertDialog confirmDialog = new android.app.AlertDialog.Builder(Setting.this).create();
                    ...
                    connectButton.setOnClickListener(v -> {
                        ...
                        if (!TextUtils.isEmpty(extUser) && !TextUtils.isEmpty(extPass) && !TextUtils.isEmpty(extCompany) && !TextUtils.isEmpty(extLicence)){
                            ...
                            new Thread(() -> {
                                ...
                                try {
                                    ...
                                    if (isSuccess){
                                        ...
                                        Setting.this.runOnUiThread(() -> {
                                            //check if its exist
                                            if (!isUserExist(extCompany, extLicence)){
                                                new UserDatabase(Setting.this).addNewUser(extDomain, extHost, extUser, extPass, extPort, extCompany, extLicence);
                                            }
                                        ...
                                        Setting.this.runOnUiThread(() -> {
                                            ...
                                            Toast.makeText(Setting.this, "Error Logging in", Toast.LENGTH_SHORT).show();
                                        });
                            ...
                            Toast.makeText(Setting.this, "Details Incomplete", Toast.LENGTH_SHORT).show();
    ...
    private void setIntervalDefault() {
        ...
        dataAdapterRefresh = new ArrayAdapter(Setting.this, R.layout.custom_spinner_item, refreshList);
    ...
    private void setAppIntervalDefault() {
        ...
        dataAdapterSync = new ArrayAdapter(Setting.this, R.layout.custom_spinner_item, syncList);
